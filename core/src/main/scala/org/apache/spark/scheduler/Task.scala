...
/**
  ...
  * <br>译文：Task是一个执行单元，在Spark中有两种Task，
  * <br>一种是<br>org.apache.spark.scheduler.ShuffleMapTask
  * <br>另一种是<br>org.apache.spark.scheduler.ResultTask
  * A Spark job consists of one or more stages. The very last stage in a job consists of multiple
  * ResultTasks, while earlier stages consist of ShuffleMapTasks. A ResultTask executes the task
  * and sends the task output back to the driver application. A ShuffleMapTask executes the task
  * and divides the task output to multiple buckets（桶） (based on the task's partitioner).
  * <br><br>
  * 译文：<br>一个Spark job 包含一个或者多个stage，作业中的最后一个stage包含多个ResultTask，然而早期的
  * <br>stage包含ShuffleMapTasks，一个ResultTask执行task并且发送task的输出给driver 。<br>
  * 一个ShuffleMapTask执行task并且依据task的partitioner算法将划分task的输出数据为多份（）
  *
  * <br><br><br>
  * Task子类有：<br>
  * 1）：ShuffleMapTask (org.apache.spark.scheduler)<br>
  * 2）：NotSerializableFakeTask (org.apache.spark.scheduler)<br>
  * 3）：LargeTask (org.apache.spark.scheduler)<br>
  * 4）：ResultTask (org.apache.spark.scheduler)<br>
  * 5）：FakeTask (org.apache.spark.scheduler)<br>
  * <br><br><br>
  *
