 ...
 * The env argument is used for testing.
  * <br> 解析spark-submit脚本的参数并封装，env参数用来测试
  * <br>
  * <br>
  * <br>主要是完成：
  * 解析spark-submit脚本传进来的参数并封装成SparkSubmitArguments对象，以及Spark环境变量和spark conf的加载
  *
  *
 */
private[spark] class SparkSubmitArguments(args: Seq[String], env: Map[String, String] = sys.env) {
   ...
   * Load arguments from environment variables, Spark properties etc.
    * 加载环境变量和spark属性（Spark properties）等配置
    *
   */
  ...
  /** Ensure that required fields exists. Call this only once all defaults are loaded.
    * <br> 确保需要的属性都存在，只在所有的默认被加载之后调用一次
    *
    */
