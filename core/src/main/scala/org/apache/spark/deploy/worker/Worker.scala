    ...
    Utils.tryOrExit {
      ...
      if (registered) {
        ...
        if (master != null) {
          ...
          //master = null  我们需要初始化获取master actor  然后注册
...
//            self 也是actor继承来的
  ...
  /**
    *
    * @return 返回WorkerID，生成方法："worker-%s-%s-%d".format(createDateFormat.format(new Date), host, port)
    */
...
private[spark] object Worker extends Logging {
  ...
  def startSystemAndActor(
    ...
    /**
      * Worker的actorSystem名字sparkWorker
      */
    val systemName = "sparkWorker" + workerNumber.map(_.toString).getOrElse("")
    /**
      * Worker的actor名字Worker
      */
