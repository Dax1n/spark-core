...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  /**
    ...
    * <br><br>输出RDD支持的文件系统，使用 Hadoop `OutputFormat` class支持的key，value类型
    *
    */
  ...
  /**
    ...
    * MapReduce job.
    * <br><br>
    * 使用Hadoop JobConf输出RDD到Hadoop支持的文件存储系统。<br>
    * Hadoop JobConf应该被设置OutputFormat和需要的输出路径，同样配置成Hadoop MapReduce 作业
    *
    */
  def saveAsHadoopDataset(conf: JobConf) {
    ...
    /**
      * 定义一个函数，函数参数：val writeToFile = (context: TaskContext, iter: Iterator[(K, V)])
      */
    val writeToFile = (context: TaskContext, iter: Iterator[(K, V)]) => {//TODO 定义一个函数
      val config = wrappedConf.value
      ...
      bytesWrittenCallback.foreach { fn => outputMetrics.setBytesWritten(fn()) }
      outputMetrics.setRecordsWritten(recordsWritten)
    }//TODO writeToFile定义结束


    //TODO 开始提交任务，self就是DAG中最后一个RDD，这个RDD通过依赖关系（宽窄依赖）进行切分Stage。
    self.context.runJob(self, writeToFile)//TODO self.context就是sparkContext
    writer.commitJob()
  }
